...
def predict(
    ...
    prediction_features = outputs["features"].cpu()[0] # (900,256)
    text_dict = outputs["text_dict"]
    encoded_text = text_dict['encoded_text'].cpu()[0] # (12,256)
    
    mask = prediction_logits.max(dim=1)[0] > box_threshold
    logits = prediction_logits[mask]  # logits.shape = (n, 256)
    boxes = prediction_boxes[mask]  # boxes.shape = (n, 4)
    features = prediction_features[mask] # (n,256)
    ...
    for i in range(len(logits)):
        ...
        phrases.append(tokenizer.decode(token_ids).replace('.', ''))
        non_zero_idx_list.append(non_zero_idx)

    return boxes, logits.max(dim=1)[0], phrases, features, encoded_text, non_zero_idx_list, logits
